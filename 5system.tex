...
This section is dedicated to design of the system that will be used to implement ITS in the
...
architecture, which will ensure there will be optimal balance between robust, reactive
behaviour without giving up capabilities to model complex behaviour.
...
\begin{itemize}
    \item Contain definitions of which skills to enable to complete a rudimentary task.
...
The layer interact with both the lower (skill) layer - interacting through events triggered by
...
The DENM service message specification is presented on the figure (\ref{denm-spec}) below.
...
agents have a goal not to crash and keep a certain speed. To ensure a fail-safe behaviour, 
a logic defined on the deliberation level is implemented to attempt to avoid object in case the drive task
fails. Furthermore, if the avoid object task fails due to another vehicle detected, the negotiation 
task is executed. The execution of this task on one agent should trigger execution of such task on the 
...
\begin{enumerate}
    ...
    \texttt{drive} task fail and (\texttt{A})'s planner has to re-plan to reach its goal (not
